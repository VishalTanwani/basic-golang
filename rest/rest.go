...
func First(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintln(w, "Get all questions with all the details")
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w)
	fmt.Fprintln(w, "id  ||  view_count  ||  answer_count  ||  comment_count  ||  favourite_count  ||  closed_date  ||  title")
	fmt.Fprintln(w)
	fmt.Fprintln(w)
...
func Second(w http.ResponseWriter, r *http.Request, postId string) {
	fmt.Fprintln(w, "Get all comments for a post")
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w)
	fmt.Fprintf(w, "id  ||  post_id  ||  creation_date  ||  text  ||  user_id  ||  score  ||  user_display_name")
	fmt.Fprintln(w)
	fmt.Fprintln(w)
